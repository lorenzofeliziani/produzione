 document.addEventListener('DOMContentLoaded', function () {
    const printBtn = document.getElementById('print-orders2');
    ...
    const editBtn   = document.getElementById('edit-orders2');
  ...
  // Optional: printWindow.close();
    ...
    const ordine = editBtn.dataset.ordine;
    if (!ordine) return;

    const header = document.querySelector(`.group-header[data-group-id="${ordine}"]`);
    if (!header) return;
    const rows = Array.from(document.querySelectorAll(`.group-row[data-group-id="${ordine}"]`));
    if (rows.length === 0) return;

    const cliente = header.dataset.cliente;    

      const data = rows[1].children[1].textContent.trim();
      ...
      rows.slice(1).forEach(row => {
        ...
        const nRiga = cells[2].textContent.trim();
        const articolo = cells[5].textContent.trim();
        const oldCode = cells[6].textContent.trim();
        const desArticolo = cells[7].textContent.trim();
        const qtaOrd = cells[12].textContent.trim();
        const operatore = cells[4].textContent.trim();
