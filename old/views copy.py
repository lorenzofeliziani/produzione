...
def dashboard(request):
    ...
    operatori = list(
        Utenti.objects.values_list('nome', flat=True).distinct().order_by('nome')
        )
    ...
    if request.method == 'POST':
        ...
        if action == "general_update":
            ...
            try:
                ...
                if stato_obj and operatore_obj:
                    Avanzamento_Ordini.objects.filter(ordine=numero_ordine).update(
                        operatore=operatore_obj, 
                        stato_ord=stato_obj       
                    )
                    for ordine in avanzamento_ordini:
                        if ordine['ordine'] == numero_ordine:
                            ordine['operatore'] = operatore_obj.username
                            ordine['des_operatore'] = operatore_obj.nome
                            ordine['id_stato_ord'] = stato_obj.id
                            ordine['des_stato_ord'] = stato_obj.stato
            ...
            try:
                ...
                if stato_obj and operatore_obj:
                    Avanzamento_Ordini.objects.filter(ordine=numero_ordine, n_riga=riga).update(
                        operatore=operatore_obj, 
                        stato_ord=stato_obj       
                    )
                    for ordine in avanzamento_ordini:
...
def select_avanzamento_ordini(request, ruolo_utente):
    ...
    risultati.sort(
    key=lambda x: datetime.fromisoformat(x['data_ord']).date() if x['data_ord'] else date.min,
    reverse=True)
